...
(function($, window, document, undefined) {
    var JxCombo = {
        init: function() {
            ...
            for (var index = 0; index < jxcombos.length; index++) {
                ...
                if (options.parent === undefined) self.fillComboIndependent($jxcombo, options); else self.bindEvents($jxcombo, options, $(options.parent));
            }
        },
        ...
        fillComboDependent: function(e) {
            var self = e.data.self, $parentCombo = $(this), $jxcombos = e.data.$jxcombos, options = e.data.options;
        ...
        fillJxCombo: function($jxcombo, options, parentValue) {
            var self = this, select = '<option value=""></option>', source = options.source;
            self.fetch($jxcombo, source, parentValue).done(function(response) {
                var defaultValue = $jxcombo.data("default");
                for (var i in response) select += '<option value="' + i + '" ' + (defaultValue == i ? "selected" : "") + ">" + response[i] + "</option>";
                $jxcombo.html(select);
    ...
    $.fn.jxcombo = function(options) {
        return this.each(function() {
            var jxcombo = Object.create(JxCombo), $this = $(this);
            options = jxcombo.getOptions($this, options);
            if (options.parent === undefined) jxcombo.fillComboIndependent($this, options); else jxcombo.bindEvents($this, options, $(options.parent));
        });
    };
    $.fn.jxcombo.options = {
        source: null
